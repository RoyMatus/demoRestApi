...
import net.datafaker.providers.entertainment.Lebowski;
...
public class LoadDatabase {
    ...
    Lebowski fakerInstance = new Faker().lebowski();

    @Bean
    CommandLineRunner initDatabase(RecordRepository repository) {
        return args -> {
            for (int i = 0; i < 100; i++) {
                repository.save(new Record(fakerInstance.character(), fakerInstance.quote()));
            }
        };
    }
